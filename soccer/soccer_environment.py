...
class SoccerEnvironment(Environment):
  ...
  The computer agent has 4 strategies according to the scenario described in the
  paper. The internal algorithm of either approaching or avoiding is by randomly
  moving the direction in either axis so that the Euclidean distance from the
  target is further.
  ...
  * "Intercept goal": See where the player is, approach him.
  ...
  The two agents move in random order, i.e., everytime the player moves, the
  computer agent might move first.
  """
  # State
  state = None
