...
class SoccerEnvironment(environment.Environment):
  ...
  def update_agent_pos(self, agent_ind, pos):
    if pos in self.soccer_pos.walkable:
      ...
      if pos == self.state.get_agent_pos(other_agent_index):
...
class SoccerState(object):
  ...
  def randomize(self):
    ...
    for agent_ind in range(len(self.spawn_bounds_list)):
      ...
      self.set_agent_pos(agent_ind, agent_pos)
  ...
  def is_agent_win(self, index):
    ...
    has_ball = self.get_agent_ball(index)
    if not has_ball:
      return False
    index_to_goal_name = {
        0: 'player',
  ...
  def get_agent_pos(self, index):
    return self.agent_list[index]['pos']

  def set_agent_pos(self, index, pos):
    self.agent_list[index]['pos'] = pos

  def get_agent_ball(self, index):
    return self.agent_list[index]['ball']
  ...
  def switch_ball(self):
    if self.get_agent_ball(0):
      self.set_agent_ball(0, False)
      self.set_agent_ball(1, True)
    else:
      self.set_agent_ball(0, True)
      self.set_agent_ball(1, False)
